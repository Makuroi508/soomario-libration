...
def run(coins, notional_frac=NOTIONAL_FRAC, leverage=LEVERAGE, friction_pct=FRICTION_PCT,
    ...
    for t in timeline:
        ...
        for coin in list(open_pos.keys()):
            ...
            # CONSERVATIVE FILL ORDER (no look-ahead): test this bar's adverse
            # extreme against the stop implied by the peak as of the END OF THE
            # PRIOR bar. The original code set peak := this bar's high and then
            # tested this bar's low against it, which assumes the high always
            # precedes the low within the bar and books ~(high - band) on nearly
            # every bar whose range exceeds the 0.55% trail band.
            if p.trail_active:
            ...
            # survived the bar -> now roll the peak forward and arm the trail
